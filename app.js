  ...
  switch (true) {
    ...
    case hasCategoryAndStatusProperties(request.query):
      if (
        ...
        if (
          ...
          getTodosQuery = `SELECT * FROM todo WHERE priority = '${category}' AND status = '${status}';`;
          data = await db.all(getTodosQuery);
          response.send(
            data.map((eachItem) => convertDataIntoResponseObject(eachItem))
          );
    ...
    case hasCategoryProperty(request.query):
      if (
        ...
        getTodosQuery = `
                SELECT * FROM todo WHERE priority = '${category}';`;
        data = await db.all(getTodosQuery);
        response.send(
          data.map((eachItem) => convertDataIntoResponseObject(eachItem))
        );
    ...
    case hasCategoryAndStatusProperties(request.query):
      if (
        ...
        if (
          ...
          getTodosQuery = `SELECT * FROM todo WHERE priority = '${category}' AND status = '${priority}';`;
          data = await db.all(getTodosQuery);
          response.send(
            data.map((eachItem) => convertDataIntoResponseObject(eachItem))
          );
...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    response.send(
      dpResponse.map((eachItem) => convertDataIntoResponseObject(eachItem))
    );
  } else {
    response.status(400);
